...
import os
import time
os.chdir("/Users/owenreiss/Desktop/python")
...
try:
    for secret_idx, secret in enumerate(all_strs):
        end_time = time.perf_counter()
        print(secret_idx, secret, round(end_time - original_time, 3))
        original_time = time.perf_counter()
        guesses = {}
        done = False
        guess = "0123"
        ...
        while not done:
            ...
            guess_tablebase[frozendict.frozendict(guesses)] = guess
            if result == (4, 0):
                done = True
                continue
            ...
            if amnt_possible <= 2:
                ...
                continue
            if frozendict.frozendict(guesses) in guess_tablebase:
                guess = guess_tablebase[frozendict.frozendict(guesses)]
                continue
            ...
            for possible_guess in all_strs:
                ...
                count += 1
            guess = best_guess
    print("Successful completion of all secrets.")
    with open("mastermind_tablebase", "wb") as file:
        pickle.dump(guess_tablebase, file)
    print("Successful dump, program has exited.")
except BaseException as the_exception:
    with open("mastermind_tablebase", "wb") as file:
        pickle.dump(guess_tablebase, file)
    print(f"Exited early, successful dump, {secret_idx} secrets complete.")
    print(f"Exception type: {type(the_exception)}")
    print(the_exception)
